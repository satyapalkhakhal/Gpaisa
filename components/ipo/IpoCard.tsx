...
export default function IpoCard({ ipo, latestGmp }: { ipo: Ipo; latestGmp?: GmpBoardEntry['latestGmp'] }) {
    return (
        <Link
            href={`/ipo/${ipo.slug}`}
            className="block bg-white rounded-xl border border-gray-100 hover:border-primary-200 hover:shadow-md transition-all p-4"
        >
            <div className="flex items-start justify-between gap-2 mb-2">
                <h3 className="font-bold text-gray-900 leading-snug">
                    {ipo.company?.name || 'Company'} <span className="font-normal text-gray-500">IPO</span>
                </h3>
